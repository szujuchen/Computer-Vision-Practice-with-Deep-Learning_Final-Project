...
class CustomizedSegVGDataset(data.Dataset):
    def __init__(self, data_root, transform=None, return_idx=False, testmode=False,
                 max_query_len=128, bert_model='bert-base-uncased', split="train"):
        self.query_len = max_query_len
        self.transform = transform
        self.testmode = testmode
        self.tokenizer = BertTokenizer.from_pretrained(bert_model, do_lower_case=True)
        ...
        with open(osp.join(self.dataset_root, f'annotations_{split}.json'), "r") as f:
            self.images = json.load(f)
        
    def pull_item(self, idx):
        img_file = self.images[idx]["img_path"]
        bbox = self.images[idx]["box"]
        phrase = self.images[idx]["caption"]
...
class TestSegVGDataset(data.Dataset):
    def __init__(self, img_path, query, transform=None, max_query_len=128, lstm=False, bert_model='bert-base-uncased'):
        self.query = query
    ...
    def pull_item(self, idx):
        try:
            ...
            print(f'error: {self.img_path} not exists')
            exit(0)
        
        return img, img, self.query, None

    def __len__(self):
        return 1

    def __getitem__(self, idx):
        ori_img, img, phrase, bbox = self.pull_item(idx)
        phrase = phrase.lower()
        input_dict = {'img': img, 'box': bbox, 'text': phrase}
        input_dict = self.transform(input_dict)
        img = input_dict['img']
        ...
        return ori_img, img, np.array(img_mask), np.array(word_id, dtype=int), np.array(word_mask, dtype=int), None, None
